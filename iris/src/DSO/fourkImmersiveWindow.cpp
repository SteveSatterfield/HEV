...
/*
 * This code was copied from immersive.cpp and modifed for a generic 4K display.
 */
...
#define fourkFront_SN   (100)
#define fourkLeft_SN    (100)
#define fourkFloor_SN   (100)

#define fourkBack_SN    (100)
#define fourkRight_SN   (100)
#define fourkCeiling_SN (100)
...
#define PIX_PER_FOURK    CONCAT ( SCR_NAME , PixelPerfourkUnit)
...
namespace CLASS_NAME
{
    ...
    int fullDisplayPixelDim[2] = {3840, 2160};


    // Viewports: x, y, w, h
    int fourkFrontVp[4] =   {0, 0, 3840, 2160};
    int fourkLeftVp[4] =    {0, 0, 3840, 2160};
    int fourkFloorVp[4] =   {0, 0, 3840, 2160};
    int fourkBackVp[4] =    {0, 0, 3840, 2160};
    int fourkRightVp[4] =   {0, 0, 3840, 2160};
    int fourkCeilingVp[4] = {0, 0, 3840, 2160};

    int fourkFrontCubeVp[4] =   {0, 0, 512, 512};
    int fourkLeftCubeVp[4] =    {0, 0, 512, 512};
    int fourkFloorCubeVp[4] =   {0, 0, 512, 512};
    int fourkBackCubeVp[4] =    {0, 0, 512, 512};
    int fourkRightCubeVp[4] =   {0, 0, 512, 512};
    int fourkCeilingCubeVp[4] = {0, 0, 512, 512};

    int fourkFrontCubeOverlapVp[4] =   {0, 0, 768, 768};
    int fourkLeftCubeOverlapVp[4] =    {0, 0, 768, 768};
    int fourkFloorCubeOverlapVp[4] =   {0, 0, 768, 768};
    int fourkBackCubeOverlapVp[4] =    {0, 0, 768, 768};
    int fourkRightCubeOverlapVp[4] =   {0, 0, 768, 768};
    int fourkCeilingCubeOverlapVp[4] = {0, 0, 768, 768};

    // Actual physical dimensions of the screens (viewport).
    // In this case we're using inches.
    double fourkFrontSize[2] =   { 36., 24.0 };
    double fourkBackSize[2] =    { 36., 24.0 };
    double fourkLeftSize[2] =    { 36., 24.0 };
    double fourkRightSize[2] =   { 36., 24.0 };
    double fourkFloorSize[2] =   { 36., 24.0 };
    double fourkCeilingSize[2] = { 36., 24.0 };

    // Pseudo-physical dimensions of the cube displays. We think of 
    // them as being 8 foot (96 inch) squares.
    double fourkFrontCubeSize[2] =   { 96.0, 96.0 };
    double fourkBackCubeSize[2] =    { 96.0, 96.0 };
    double fourkLeftCubeSize[2] =    { 96.0, 96.0 };
    double fourkRightCubeSize[2] =   { 96.0, 96.0 };
    double fourkFloorCubeSize[2] =   { 96.0, 96.0 };
    double fourkCeilingCubeSize[2] = { 96.0, 96.0 };

    double fourkFrontCubeOverlapSize[2] =   { 96.0, 96.0 };
    double fourkBackCubeOverlapSize[2] =    { 96.0, 96.0 };
    double fourkLeftCubeOverlapSize[2] =    { 96.0, 96.0 };
    double fourkRightCubeOverlapSize[2] =   { 96.0, 96.0 };
    double fourkFloorCubeOverlapSize[2] =   { 96.0, 96.0 };
    double fourkCeilingCubeOverlapSize[2] = { 96.0, 96.0 };


    // Physical length of one display unit (RAVE coord unit)
    // Same units should be used as for Size params.
    // In this case we're using inches.
    double sizeOfOnefourkUnit = 18.0; 

    // Current situation is that all screens have same pixel/rave unit.
    double PixelPerfourkUnit[2];  // This will be calculated below
    double *fourkFrontPixelPerfourkUnit = PixelPerfourkUnit;
    double *fourkBackPixelPerfourkUnit = PixelPerfourkUnit;
    double *fourkLeftPixelPerfourkUnit = PixelPerfourkUnit;
    double *fourkRightPixelPerfourkUnit = PixelPerfourkUnit;
    double *fourkFloorPixelPerfourkUnit = PixelPerfourkUnit;
    double *fourkCeilingPixelPerfourkUnit = PixelPerfourkUnit;
    double *powerwallPixelPerfourkUnit = PixelPerfourkUnit;

    // These offsets describe how the simulator (and cube) windows are 
    // positioned relative to one another on DISPLAY 0.0.
    double fourkFrontSimWinOffset[2] =   {1.0, 0.0};
    double fourkBackSimWinOffset[2] =    {1.1, 0.1};
    double fourkLeftSimWinOffset[2]  =   {0.0, 0.0};
    double fourkRightSimWinOffset[2]  =  {0.1, 0.1};
    double fourkFloorSimWinOffset[2] =   {0.0, 1.0};
    double fourkCeilingSimWinOffset[2] = {0.1, 1.1};
    ...
    public:
        CLASS_NAME ():iris::Augment(CLASS_NAME_STR) 
        {
            ...
            osg::Quat fourkFrontOrientation = iris::EulerToQuat(0.f, 0.f, 0.f);
            osg::Quat fourkLeftOrientation = iris::EulerToQuat(90.f, 0.f, 0.f);
            osg::Quat fourkFloorOrientation= iris::EulerToQuat(90.f, -90.f, 0.f);

            osg::Quat fourkBackOrientation = iris::EulerToQuat(180.f, 0.f, 0.f);
            osg::Quat fourkRightOrientation = iris::EulerToQuat(-90.f, 0.f, 0.f);
            osg::Quat fourkCeilingOrientation=iris::EulerToQuat(90.f, 90.f, 0.f);
            ...
            double frontLeftFloorCornerfourkCoords[3] = {-1.0, 1.0, -1.0};
            ...
            PixelPerfourkUnit[0] = VP[2] / (SCR_SIZE[0]/sizeOfOnefourkUnit);
            PixelPerfourkUnit[1] = VP[3] / (SCR_SIZE[1]/sizeOfOnefourkUnit);
            ...
            osg::Vec3 fourkFrontCenter = 
              osg::Vec3 ( 
                frontLeftFloorCornerfourkCoords[0] + 
                                (fourkFrontSize[0]/2.0)/sizeOfOnefourkUnit, 
                frontLeftFloorCornerfourkCoords[1],
                frontLeftFloorCornerfourkCoords[2] + 
                                (fourkFrontSize[1]/2.0)/sizeOfOnefourkUnit
                );

            osg::Vec3 fourkLeftCenter = 
              osg::Vec3 ( 
                frontLeftFloorCornerfourkCoords[0],
                frontLeftFloorCornerfourkCoords[1] -
                                (fourkLeftSize[0]/2.0)/sizeOfOnefourkUnit, 
                frontLeftFloorCornerfourkCoords[2] + 
                                (fourkLeftSize[1]/2.0)/sizeOfOnefourkUnit
                );
            ...
            osg::Vec3 fourkFloorCenter = 
              osg::Vec3 ( 
                frontLeftFloorCornerfourkCoords[0] +
                                (fourkFloorSize[1]/2.0)/sizeOfOnefourkUnit,
                frontLeftFloorCornerfourkCoords[1] -
                                (fourkFloorSize[0]/2.0)/sizeOfOnefourkUnit, 
                frontLeftFloorCornerfourkCoords[2]
                );



            osg::Vec3 fourkBackCenter = 
              osg::Vec3 ( 
                         fourkFrontCenter[0],
                         -1*fourkFrontCenter[1],
                         fourkFrontCenter[2]
            ...
            osg::Vec3 fourkCeilingCenter = 
              osg::Vec3 ( 
                         fourkFloorCenter[0],
                         fourkFloorCenter[1],
                         -1*fourkFloorCenter[2]
            ...
            osg::Vec3 fourkFrontCubeCenter   = osg::Vec3 ( 0.0,  1.0,  0.0);
            osg::Vec3 fourkBackCubeCenter    = osg::Vec3 ( 0.0, -1.0,  0.0);
            osg::Vec3 fourkLeftCubeCenter    = osg::Vec3 (-1.0,  0.0,  0.0);
            osg::Vec3 fourkRightCubeCenter   = osg::Vec3 ( 1.0,  0.0,  0.0);
            osg::Vec3 fourkFloorCubeCenter   = osg::Vec3 ( 0.0,  0.0, -1.0);
            osg::Vec3 fourkCeilingCubeCenter = osg::Vec3 ( 0.0,  0.0,  1.0);

            osg::Vec3 fourkFrontCubeOverlapCenter   = osg::Vec3 ( 0.0,  1.0,  0.0);
            osg::Vec3 fourkBackCubeOverlapCenter    = osg::Vec3 ( 0.0, -1.0,  0.0);
            osg::Vec3 fourkLeftCubeOverlapCenter    = osg::Vec3 (-1.0,  0.0,  0.0);
            osg::Vec3 fourkRightCubeOverlapCenter   = osg::Vec3 ( 1.0,  0.0,  0.0);
            osg::Vec3 fourkFloorCubeOverlapCenter   = osg::Vec3 ( 0.0,  0.0, -1.0);
            osg::Vec3 fourkCeilingCubeOverlapCenter = osg::Vec3 ( 0.0,  0.0,  1.0);
		...
		sgs_scr_num=3;
	      ...
	      // end of hack
            ...
            osg::Vec2 vpExtent = osg::Vec2 ( VP[2]/PIX_PER_FOURK[0], 
                                             VP[3]/PIX_PER_FOURK[1] ) ;
